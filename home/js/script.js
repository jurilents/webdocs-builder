...
showMoreButtons.forEach((button) => {
    button.addEventListener('click', (event) => {
        event.preventDefault()
        const targets = button.parentNode.querySelectorAll('[data-target-for-slide-button]')

        changeClass([...targets], '_opened')
    })
})

function changeClass(elementOrElements, newClass) {
    if (Array.isArray(elementOrElements)) {
        elementOrElements.forEach((element) => {
            element.classList.toggle(newClass)
        })
    } else {
        elementOrElements.classList.toggle(newClass)
    }
}

//  Context menu

const contextStructureFlags = document.querySelectorAll('[data-context-menu-structure]')
contextStructureFlags.forEach((flag) => {
    ContextMenu.start({
        strucrtureOfContextMenu: flag.dataset['contextMenuStructure'],
        parentElem: flag,
    })
})
